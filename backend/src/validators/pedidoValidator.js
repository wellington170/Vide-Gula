...
const createOrderRules = [
  ...
  body('items.*.produtoId').isInt().withMessage('ID do produto inválido.'),
  ...
  body('enderecoId').optional().isInt().withMessage('ID do endereço inválido.')
];

const updateOrderRules = [
  param('id').isInt().withMessage('ID do pedido inválido.'),
  body('items').optional().isArray({ min: 1 }).withMessage('O pedido deve possuir pelo menos um item.'),
  body('items.*.produtoId').optional().isInt().withMessage('ID do produto inválido.'),
